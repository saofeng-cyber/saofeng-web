...
<template>
  <article class="mx-auto prose prose-slate md:prose-lg lg:prose-xl">
    ...
    <ul>
      <li>
        浅拷贝：对于基本数据类型拷贝是完整的值，对于引用数据类型，拷贝是内存地址，如果对象中的某个值改变，会影响另一个对象
      </li>
      <li>
        深拷贝：重新开辟了一个堆内存，在新对的区域存放新对象，切修改新对象不会影像原对象
      </li>
    </ul>
    <pre>
    ...
    <h3>原型链</h3>
    <p>
      每个函数身上都有一个 prototype
      的原型对象，并且有一个__proto__的指针指向下一级原型对象，如果一个对象的属性或方法在自身中找不到，
      那么就会去 prototype 原型对象中查找，如果还找不到继续向上查找直到
      null，当_proto_指针指向 null 时形成一个链条，这个链条叫做原型链。
      在原型链中，对象可以继承原型对象的属性和方法。如果想在后面所有构造函数中添加某些属性和方法，可以将它们添加到构造函数的
      prototype 属性中，
