...
export function modelTree(
  ...
  const config = {
    id: id || 'id',
    belongModel: belongModel || 'belongModel',
    childrenList: children || 'children'
  };
  ...
  for (const d of data) {
    const belongModel = d[config.belongModel];
    if (childrenListMap[belongModel] == null) {
      childrenListMap[belongModel] = [];
    }
    nodeIds[d[config.id]] = d;
    childrenListMap[belongModel].push(d);
  }

  for (const d of data) {
    const belongModel = d[config.belongModel];
    if (nodeIds[belongModel] == null) {
      tree.push(d);
    }
  }
...
export function isKeyEmpty(obj: { [x: string]: string | number }) {
