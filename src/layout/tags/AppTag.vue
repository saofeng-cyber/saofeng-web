...
<template>
    <div class="tags-views" style="--n-color: #1890ff;--n-border-hover: #1890ff">
        <div class="tags" v-for="item in tabsList" :key="item.fullPath"
            :style="{ color: state.activeKey === item.fullPath ? '#1890ff' : '#333333', border: state.activeKey === item.fullPath ? '1px solid #1890ff' : '1px solid #f0f0f0' }">
            <router-link class="tag" :to="item.fullPath">
                {{ item.meta.title }}
            </router-link>
            <n-icon v-if="!item.meta.affix" size="16" @click="removeTab(item)"
